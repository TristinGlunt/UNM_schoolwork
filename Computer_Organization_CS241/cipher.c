...
/*      Tristin Glunt                     */
/*      CS 241 - Lab 6                    */
/*      Cipher program to decrypt         */    
/*      and encrypt given lines depending */
/*      on structure of                   */
/******************************************/
...
int readUL();
int finishLine();
void encrypt(struct LinearCongruentialGenerator *lcg);
...
test function to help me figure out if the data reading was starting
at the right part of the header
int readData()
{
    int c;
    while((c = getchar()) != '\n')
...
int decrypt(struct LinearCongruentialGenerator *lcg)
{
    ...
    while((b = getchar()) != '\n')
    {
        ...
        x = getNextRandomValue(lcg);
        decryptedByte = b ^ (x % 128);
        if(decryptedByte > 127 || decryptedByte < 32)
        {
          return ERROR;
        }
        else{ putchar(decryptedByte);}
    }
    printf("\n");
    /*printf("will decrypt\n");*/
}
