...
void getFileFreq(FILE* in, FILE* out)
{
  ...
  unsigned long specLowerFreq[33] = {0};
  unsigned long specHigherFreq[128] = {0};
  unsigned long capitalFreq[26] = {0};
  ...
  {
    unsigned long fileCharacter = (unsigned long)fileCh;
    if(fileCharacter >= 'a' && fileCharacter <= 'z')
    {
      freq[fileCharacter - 'a']++;
      totalCount++;
    }
    else if(fileCharacter >= 'A' && fileCharacter <= 'Z')
    {
      capitalFreq[fileCharacter - 'A']++;
      totalCount++;
    }
    else if(fileCharacter > 'Z')
    {
      specHigherFreq[fileCharacter]++;
      totalCount++;
    }
    else
    {
      specLowerFreq[fileCharacter]++;
      totalCount++;
    }
  }
  for(z = 0; z < 128; z++)
  {
    if(specHigherFreq[z] == 0)
      continue;
    else
    {
      printf("=%d\t%lu\n", z, specHigherFreq[z]);
